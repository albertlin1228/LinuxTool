...
def CheckBarRegister(HexDumpArray,DeviceType):
    ...
    if (Bar0Value & CommonLib.BIT0) == True:
        print("[0x10][Bit 0]IO space indicator")
    else:
        if (DeviceType == 0):
            if ((Bar0Value & (CommonLib.BIT1 | CommonLib.BIT2)) >> 1) == CommonLib.BIT1:
                print("[0x10][Bit 1:2]64 bits Memory space indicator","Prefetchable" if(Prefetchable == 1) else "Non Prefetchable")
                ...
                print("[0x10][Bit 1:2]32 bits Memory space indicator","Prefetchable" if(Prefetchable == 1) else "Non Prefetchable")
...
def GetPcieCapRegister(HexDumpArray,CapHeaderOffset):
    ...
    print(f"[0x{PcieCapOffset:02X} 02]Pcie Capibilities:")
...
def GetDeviceCap(HexDumpArray,CapHeaderOffset):
    ...
    print(f"[0x{DeviceCapOffset:02X} 04]Device Capibilities:")
...
def GetDeviceControl(HexDumpArray,CapHeaderOffset):
    ...
    print(f"[0x{DeviceControlOffset:02X} 02]Device Control:")
...
def GetDeviceStatus(HexDumpArray,CapHeaderOffset):
    ...
    print(f"[0x{DeviceStatusOffset:02X} 02]Device Status:")
...
def GetLinkCap(HexDumpArray,CapHeaderOffset):
    ...
    print(f"[0x{LinkCapOffset:02X} 04]Link Capibilities:")
...
def GetLinkControl(HexDumpArray,CapHeaderOffset):
    ...
    print(f"[0x{LinkControlOffset:02X} 02]Link Control:")
...
def GetLinkStatus(HexDumpArray,CapHeaderOffset):
    ...
    print(f"[0x{LinkStatusOffset:02X} 02]Link Status:")
...
def CheckUEStatus(HexDumpArray,CapHeaderOffset):
    ...
    print(f"[0x{UEStatusOffset:04X} 04]Uncorrectable Error Status:")
